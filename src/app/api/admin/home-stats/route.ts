...
export async function GET(request: Request) {
  try {
    const { stdout, stderr } = await execAsync('df -k /home');

    if (stderr) {
      console.error(`Error getting home stats: ${stderr}`);
      return NextResponse.json({ message: '/homeディレクトリの統計の取得中にエラーが発生しました。' }, { status: 500 });
    }
